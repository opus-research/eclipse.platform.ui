/*******************************************************************************
 * Copyright (c) 2008, 2010, 2015 Matthew Hall and others.
	...
	protected void firstListenerAdded() {
		...
		knownMasterValues.addSetChangeListener(new ISetChangeListener<I>() {
			...
			public void handleSetChange(SetChangeEvent<I> event) {
				for (Iterator<I> it = event.diff.getRemovals().iterator(); it.hasNext();) {
					I key = it.next();
				...
				for (Iterator<I> it = event.diff.getAdditions().iterator(); it.hasNext();) {
					I key = it.next();

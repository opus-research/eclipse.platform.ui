...
public class ResourceExtensionContentProvider extends WorkbenchContentProvider {
	 ...
	 * resourceToUpdate.
	 */
	private void processDelta(IResourceDelta delta, Collection<Runnable> andAndRemoveRunnables,
...
//			runnables.add(updateRunnable);
		...
		for (int i = 0; i < affectedChildren.length; i++) {
			processDelta(affectedChildren[i], andAndRemoveRunnables, toRefresh);
		}
		...
		andAndRemoveRunnables.add(addAndRemove);
	}

...
public final class ActivityCategoryPreferencePage extends PreferencePage implements
	...
	/**
	 * The label to be used for the prompt button. Ie:
	 * "&Prompt when enabling capabilities".
	 */
	public static final String ACTIVITY_PROMPT_BUTTON = "activityPromptButton"; //$NON-NLS-1$

	/**
	 * The tooltip to be used for the prompt button. Ie:
	 * "Prompt when a feature is first used that requires enablement of capabilities"
	 * .
	 */
	...
	private class AdvancedDialog extends TrayDialog {
		...
		protected void configureShell(Shell newShell) {
			...
			String activityName = strings.getProperty(ACTIVITY_NAME,
			...
			newShell.setText(NLS.bind(
					ActivityMessages.ActivitiesPreferencePage_advancedDialogTitle, activityName));
		}
	...
	private class CategoryLabelProvider extends LabelProvider implements ITableLabelProvider,
		...
		public Image getColumnImage(Object element, int columnIndex) {
			...
			ImageDescriptor descriptor = PlatformUI.getWorkbench().getActivitySupport()
	...
	protected Control createContents(Composite parent) {
		...
		label.setText(strings.getProperty(CAPTION_MESSAGE,
				ActivityMessages.ActivitiesPreferencePage_captionMessage));
	...
	private void createPromptButton(Composite composite) {
		...
		activityPromptButton.setText(strings.getProperty(ACTIVITY_PROMPT_BUTTON,
				ActivityMessages.activityPromptButton));
		activityPromptButton.setToolTipText(strings.getProperty(ACTIVITY_PROMPT_BUTTON_TOOLTIP,
				ActivityMessages.activityPromptToolTip));
		...
		if (allowAdvanced) {
			...
			advancedButton.addSelectionListener(new SelectionAdapter() {
				...
				public void widgetSelected(SelectionEvent e) {
					...
					dialog.open(); // logic for updating the working copy is in
									// the dialog class.
				}
			});
	...
	private void createDetailsArea(Composite parent) {
		...
		new Label(composite, SWT.NONE)
				.setText(ActivityMessages.ActivitiesPreferencePage_requirements);
	...
	private void createCategoryArea(Composite parent) {
		...
		label.setText(strings.getProperty(CATEGORY_NAME,
				ActivityMessages.ActivityEnabler_categories) + ':');
		...
		table.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				if (e.detail == SWT.CHECK) {
					...
					Set<String> activitySet = WorkbenchActivityHelper
							.getActivityIdsForCategory(category);
					if (tableItem.getChecked()) {
						activitySet.addAll(workingCopy.getEnabledActivityIds());
					} else {
						HashSet<String> newSet = new HashSet<String>(workingCopy
	...
	protected void setDetails(ICategory category) {
		...
		if (WorkbenchActivityHelper.isEnabled(workingCopy, category.getId())) {
			categories = WorkbenchActivityHelper.getDisabledCategories(workingCopy,
					category.getId());

		} else {
			categories = WorkbenchActivityHelper
					.getEnabledCategories(workingCopy, category.getId());
		}
	...
	protected boolean isLocked(ICategory category) {
		return !WorkbenchActivityHelper.getDisabledCategories(workingCopy, category.getId())
				.isEmpty();
	}

...
public class ViewsPreferencePage extends PreferencePage implements
	...
	private List<ITheme> getCSSThemes(boolean highContrastMode) {
		...
		for (ITheme theme : engine.getThemes()) {
			/*
			 * When we have Win32 OS - we don't display the 'high-contrast'
			 * special theme when the mode is not enabled. We don't want to mess
			 * the themes combo with the theme since it is the special variation
			 * of the 'classic' theme.
			 * 
			 * When we have GTK - we have to display the entire list of the
			 * themes since we are not able to figure out the high contrast mode
			 * when GTK and the user has to manually select the theme if they
			 * need
			 */

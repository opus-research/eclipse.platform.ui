...
public class WindowBuilder {
	...
	private void create() {
		...
		window.getPersistedState().put(IPreferenceConstants.COOLBAR_VISIBLE, windowReader.getCoolbarVisible());
		window.getPersistedState().put(IPreferenceConstants.PERSPECTIVEBAR_VISIBLE, Boolean.toString(true));
	...
	private void populate() {
		...
		perspectiveStack = createPerspectiveStack();
		mainSash.getChildren().add(perspectiveStack);

		window.getChildren().add(mainSash);

		List<PerspectiveReader> memPersps = windowReader.getPerspectives();
		for (PerspectiveReader memPersp : memPersps) {
			PerspectiveBuilder builder = factory.createPerspectiveBuilder(memPersp, perspectiveStack, window);
			builder.build();
		}
	...
	private void addEditors() {
		...
		for (EditorReader editorReader : readers) {
			...
			editor.getPersistedState().put("memento", editorReader.toString()); //$NON-NLS-1$
	...
	private MPart createView(ViewReader viewReader) {
		...
		view.getPersistedState().put("memento", viewReader.getViewState()); //$NON-NLS-1$
		return view;
	}

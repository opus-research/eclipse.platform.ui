...
public class PerspectiveBuilder {
	...
	@Inject
	@Optional
	private MPerspectiveStack perspStack;
	...
	public void build() {
		create();
		perspStack.getChildren().add(perspective);
		tags = perspective.getTags();
		populate();
	}

	public void buildAsSnippet() {
		create();
	...
	private void addEditorArea(InfoReader info) {
		if (window == null) { // TODO snippet
			MPlaceholder eaRef = AdvancedFactoryImpl.eINSTANCE.createPlaceholder();
			eaRef.setElementId(ID_EDITOR_AREA);

			addToPerspective(eaRef, info);
			return;
		}
		...
		for (MUIElement element : sharedElements) {
			if (element.getElementId().equals(ID_EDITOR_AREA)) {
	...
	private void populatePartStack(MPartStack stack, InfoReader info) {
		...
		for (PageReader page : pages) {
			if (window == null) {
				...
				addPlaceholderToStack(stack, id, null);
			} else {
				addPartToStack(stack, page);
			}
		}
	...
	private void addPartToStack(MPartStack stack, PageReader page) {
		String id = page.getId();
		MPart part = null;
		...
		addPlaceholderToStack(stack, id, part);
	}

	public void addPlaceholderToStack(MPartStack stack, String partId, MPart part) {

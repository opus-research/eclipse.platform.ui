...
public class PerspectiveBuilder {
	...
	private Boolean maximizedViewExists;
	...
	private MPartStack addPartStack(InfoReader info) {
		...
		if (info.isMinimized() || (hasMaximizedView() && !info.isMaximized())) {
			stack.getTags().add(TAG_MINIMIZED);
		}
		return stack;
	}

	private boolean hasMaximizedView() {
		if (maximizedViewExists != null) {
			return maximizedViewExists;
		}
		maximizedViewExists = Boolean.FALSE;
		for (InfoReader info : perspReader.getInfos()) {
			if (info.isMaximized()) {
				maximizedViewExists = Boolean.TRUE;
				break;
			}
		}
		return maximizedViewExists;

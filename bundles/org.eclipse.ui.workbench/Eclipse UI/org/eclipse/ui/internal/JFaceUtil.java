...
final class JFaceUtil {
	...
	public static void initializeJFace() {
		...
		if (Boolean.TRUE.toString().equalsIgnoreCase(Platform.getDebugOption(Policy.JFACE + "/debug"))) { //$NON-NLS-1$
			...
			InternalPolicy.DEBUG_LOAD_URL_IMAGE_DESCRIPTOR_2x = !Boolean.FALSE.toString().equalsIgnoreCase(Platform.getDebugOption(Policy.JFACE + "/debug/loadURLImageDescriptor2x")); //$NON-NLS-1$
			InternalPolicy.DEBUG_LOAD_URL_IMAGE_DESCRIPTOR_2x_PNG_FOR_GIF = Boolean.TRUE.toString().equalsIgnoreCase(

...
public class WorkbenchThemeManager extends EventManager implements
	...
	public static class WorkbenchThemeChangedHandler implements EventHandler {
		...
		private void resetThemeRegistries(ThemeRegistry themeRegistry,
			...
			List<ThemeElementDefinition> toRemove = new ArrayList<ThemeElementDefinition>();
			for (FontDefinition fontDefinition : themeRegistry.getFonts()) {
				if (fontDefinition.isAddedByCss()) {
					toRemove.add(fontDefinition);
				} else if (fontDefinition.isOverridden()) {
					fontDefinition.resetToDefaultValue();
				}
			}
			for (ColorDefinition colorDefinition : themeRegistry.getColors()) {
				if (colorDefinition.isAddedByCss()) {
					toRemove.add(colorDefinition);
				} else if (colorDefinition.isOverridden()) {
					colorDefinition.resetToDefaultValue();
				}
			}
			for (ThemeElementDefinition definition : toRemove) {
				if (definition instanceof FontDefinition) {
					themeRegistry.remove((FontDefinition) definition);
					fontRegistry.remove(definition.getId());
				} else if (definition instanceof ColorDefinition) {
					themeRegistry.remove((ColorDefinition) definition);
					colorRegistry.remove(definition.getId());
				}
			}
		}

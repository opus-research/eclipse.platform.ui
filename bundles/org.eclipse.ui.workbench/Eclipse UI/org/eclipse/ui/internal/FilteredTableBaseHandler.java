...
public abstract class FilteredTableBaseHandler extends AbstractHandler implements
	...
	public void openDialog(WorkbenchPage page, IWorkbenchPart activePart) {
		...
		dialog = new Shell(shell, SWT.MODELESS | SWT.RESIZE);
		...
		if (isFiltered()) {
			...
			title.setText(getTableHeader(activePart));
		...
		if (isFiltered()) {
			...
			addKeyListener(text);
			text.setMessage("Filter"); //$NON-NLS-1$
		...
		if (!isFiltered()) {
			dialogBounds.height = title.getBounds().height + label.getBounds().height + tableBounds.height
					+ gl_composite.marginHeight * 2 + gl_composite.verticalSpacing * 2;
		} else {
			dialogBounds.height = text.getBounds().height + +label.getBounds().height + tableBounds.height
					+ gl_composite.marginHeight * 2 + gl_composite.verticalSpacing * 2;
		}

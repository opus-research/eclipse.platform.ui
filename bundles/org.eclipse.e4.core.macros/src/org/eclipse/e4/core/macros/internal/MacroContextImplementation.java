...
public class MacroContextImplementation implements EMacroContext {
	...
	public static final String MACRO_COMMAND_ID = "commandId"; //$NON-NLS-1$
	public static final String MACRO_COMMAND_CLASS = "class"; //$NON-NLS-1$

	private static Map<String, IMacroCommandFactory> fCommandIdToMacroFactory;
	...
	/**
	 * @return Returns the fCommandIdToMacroFactory (creates it lazily if it
	 *         still wasn't created).
	 */
	private Map<String, IMacroCommandFactory> getCommandIdToMacroFactory() {
		if (fCommandIdToMacroFactory == null && fEclipseContext != null) {
			IExtensionRegistry registry = fEclipseContext.get(IExtensionRegistry.class);

			Map<String, IMacroCommandFactory> validCommandIds = new HashMap<>();
			for (IConfigurationElement ce : registry.getConfigurationElementsFor(MACRO_COMMAND_EXTENSION_POINT)) {
				...
				if (macroCommandId != null && macroCommandClass != null) {
					try {
						...
						validCommandIds.put(macroCommandId, macroFactory);
					} catch (CoreException e) {
						Activator.log(e);
					}
				} else {
			...
			fCommandIdToMacroFactory = validCommandIds;
		}
		return fCommandIdToMacroFactory;
	}
	...
	public void toggleMacroRecord() {
		...
		getMacroManager().toggleMacroRecord(this, getCommandIdToMacroFactory());
	}
	...
	public void playbackLastMacro() {
		...
		loadExtensionPointsMacroListeners();
		getMacroManager().playbackLastMacro(this, macroPlaybackContext, getCommandIdToMacroFactory());
	}

...
public class MacroManager {
	 ...
	 */
	public void toggleMacroRecord(final EMacroContextService macroContext,
		...
		try {
			if (fMacroBeingRecorded == null) {
				fMacroBeingRecorded = new ComposableMacro(macroCommandIdToFactory);
			} else {
				try {
	 ...
	 */
	private void notifyMacroStateChange(final EMacroContextService macroContext) {
	 ...
	 *            command id.
	 */
	public void playbackLastMacro(EMacroContextService macroContext, final IMacroPlaybackContext macroPlaybackContext,
			final Map<String, IMacroCommandFactory> macroCommandIdToFactory) {
		if (fLastMacro != null && !fIsPlayingBack) {
			...
			try {
				...
				SafeRunner.run(new ISafeRunnable() {
					...
					@Override
					public void run() throws Exception {
						fLastMacro.setMacroCommandIdToFactory(macroCommandIdToFactory);

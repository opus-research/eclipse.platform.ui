...
public class SavedJSMacro implements IMacro {
	 ...
	 * recreate it.
	 */
	private Map<String, IMacroCommandCreator> fCommandIdToCreator;
	...
	public static void runCommand(Map<String, IMacroCommandCreator> commandIdToCreator,
			IMacroPlaybackContext macroPlaybackContext, String commandId, Object commandParameters) throws Exception {
		Assert.isNotNull(commandIdToCreator, "Before running, fCommandIdToCreator must be set."); //$NON-NLS-1$
		...
		IMacroCommandCreator iMacroCreator = commandIdToCreator.get(commandId);
		if (iMacroCreator == null) {
			throw new RuntimeException("Unable to find IMacroCommandCreator for command: " + commandId); //$NON-NLS-1$
		}
	...
	@Override
	public void setCommandIdToCreator(Map<String, IMacroCommandCreator> commandIdToCreator) {

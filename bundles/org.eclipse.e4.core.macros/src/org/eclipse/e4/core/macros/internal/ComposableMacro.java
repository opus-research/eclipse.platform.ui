...
/* default */ class ComposableMacro implements IMacro {
	 ...
	 *            macroCommandIdToFactory will be accepted.
	 */
	public ComposableMacro(Map<String, IMacroCommandFactory> macroCommandIdToFactory) {
		fMacroCommandIdToFactory = macroCommandIdToFactory;
	}
	...
	public void addMacroCommand(IMacroCommand macroCommand) {
		if (fMacroCommandIdToFactory != null && !fMacroCommandIdToFactory.containsKey(macroCommand.getId())) {
			throw new RuntimeException(
					String.format("Command: %s not properly registered through a %s extension point.", //$NON-NLS-1$
							macroCommand.getId(), MacroContextServiceImplementation.MACRO_COMMAND_EXTENSION_POINT));
		}

 ...
 */
public class CancelabilityPreferencePage extends FieldEditorPreferencePage
	...
	private class IntegerEditor extends IntegerFieldEditor {
		...
		protected boolean checkState() {
			...
			if (preferenceName.equals(PreferenceConstants.TASK_MONITORING_ERROR_THRESHOLD_MILLIS)) {
				if (maxTimeWarningThreshold.isValid() &&
	...
	public CancelabilityPreferencePage() {
		super(GRID);
		editors = new HashMap<>();
	}
	...
	private Composite createTopBlock(Composite container) {
		...
		monitoringEnabled = createBooleanEditor(PreferenceConstants.TASK_MONITORING_ENABLED,
				Messages.CancelabilityMonitorPreferencePage_enable_monitoring_label, block);

		maxTimeWarningThreshold = createIntegerEditor(
				PreferenceConstants.TASK_MONITORING_WARNING_THRESHOLD_MILLIS,
				Messages.CancelabilityMonitorPreferencePage_warning_threshold_label, block,
				3, HOUR_IN_MS);
		maxTimeErrorThreshold = createIntegerEditor(
				PreferenceConstants.TASK_MONITORING_ERROR_THRESHOLD_MILLIS,
				Messages.CancelabilityMonitorPreferencePage_error_threshold_label, block,
				3, HOUR_IN_MS);
		createIntegerEditor(
				PreferenceConstants.TASK_MONITORING_MAX_STACK_SAMPLES,
				Messages.CancelabilityMonitorPreferencePage_max_stack_samples_label, block, 0, 100);

		createBooleanEditor(PreferenceConstants.TASK_MONITORING_LOG_NON_CANCELLABLE_USER_JOB,
				Messages.CancelabilityMonitorPreferencePage_log_non_cancelable_user_job_label, block);
	...
	public void propertyChange(PropertyChangeEvent event) {
        if (event.getProperty().equals(FieldEditor.VALUE)) {
    		...
    		if (source instanceof FieldEditor) {
				...
				if (preferenceName.equals(PreferenceConstants.TASK_MONITORING_ENABLED)) {
	...
	private void enableDependentFields(boolean enable) {
		for (Map.Entry<FieldEditor, Composite> entry : editors.entrySet()) {
			...
			if (!editor.getPreferenceName().equals(PreferenceConstants.TASK_MONITORING_ENABLED)) {
	...
	private <T extends FieldEditor> T addField(T editor, Composite parent) {
		...
		if (!editor.getPreferenceName().equals(PreferenceConstants.TASK_MONITORING_ENABLED)) {
			boolean enabled = preferences.getBoolean(PreferenceConstants.TASK_MONITORING_ENABLED);

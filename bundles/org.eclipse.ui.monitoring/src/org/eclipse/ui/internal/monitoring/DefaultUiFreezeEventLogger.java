...
public class DefaultUiFreezeEventLogger implements IUiFreezeEventLogger {
	...
	public void log(UiFreezeEvent event) {
		...
		for (int i = 0; i < event.getSampleCount(); i++) {
			...
			// Grab the stack trace of only the main thread.
			for (ThreadInfo thread : threads) {
				...
				if (thread.getThreadName().equals("main")) { //$NON-NLS-1$
					traceStatus = new MultiStatus(PreferenceConstants.PLUGIN_ID,
							IStatus.INFO, traceText, stackTrace);
					loggedEvent.add(traceStatus);
					break;
				}
			}
			
			// Grab stack traces of remaining threads.
			if (threads.length > 1) {
				for (ThreadInfo thread : threads) {
					if (thread.getThreadName().equals("main")) { //$NON-NLS-1$
						continue;

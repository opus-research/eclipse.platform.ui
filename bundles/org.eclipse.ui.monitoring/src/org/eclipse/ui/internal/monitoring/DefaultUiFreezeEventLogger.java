...
public class DefaultUiFreezeEventLogger implements IUiFreezeEventLogger {
	...
	private static class InfoMultiStatus extends MultiStatus {
		public InfoMultiStatus(String pluginId, String message, Throwable exception) {
			super(pluginId, INFO, message, exception);
			setSeverity(INFO);
		}
	}
	...
	public void log(UiFreezeEvent event) {
		...
		MultiStatus loggedEvent = new InfoMultiStatus(PreferenceConstants.PLUGIN_ID, header, null);
		...
		for (int i = 0; i < event.getSampleCount(); i++) {
			...
			MultiStatus traceStatus = new InfoMultiStatus(PreferenceConstants.PLUGIN_ID,

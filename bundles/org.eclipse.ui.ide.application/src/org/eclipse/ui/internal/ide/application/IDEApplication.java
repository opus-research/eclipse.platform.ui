...
public class IDEApplication implements IApplication, IExecutableExtension {
    ...
    // to change on an update of the IDE.  We have set a dependency on it so we know
    // it's there when we run.
    private static final String WORKSPACE_CHECK_REFERENCE_BUNDLE_NAME = "org.eclipse.platform"; //$NON-NLS-1$
	private static final Version WORKSPACE_CHECK_REFERENCE_BUNDLE_VERSION = Platform.getBundle(WORKSPACE_CHECK_REFERENCE_BUNDLE_NAME).getVersion();

    private static final String WORKSPACE_CHECK_REFERENCE_BUNDLE_NAME_LEGACY = "org.eclipse.core.runtime"; //$NON-NLS-1$
    private static final int WORKSPACE_CHECK_LEGACY_VERSION = 1;
    private static final int WORKSPACE_CHECK_LEGACY_VERSION_UPDATED = WORKSPACE_CHECK_LEGACY_VERSION + 1;

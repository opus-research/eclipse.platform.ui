...
public class ProgressMonitorPart extends Composite implements
	...
	/**
	 * Creates a <code>ProgressMonitorPart</code> that does not provide a stop
	 * button.
	 *
	 * @param parent
	 *            The SWT parent of the part.
	 * @param layout
	 *            The SWT grid layout used by the part. A client can supply the
	 *            layout to control how the progress monitor part is laid out.
	 *            If <code>null</code> is passed the part uses its default
	 *            layout.
	 */
	 ...
	 *            be SWT.DEFAULT in order to get the default height as
	 *            calculated by the widget and its layout.
	 */
	public ProgressMonitorPart(Composite parent, Layout layout,
	 ...
	 *            <code>true</code> if the progress indicator should include a
	 *            stop button that can be used to cancel any currently running
	 *            task, and <code>false</code> if no such stop button should be
	 *            created.
	 * 
	 * @since 3.6
	 */
	 ...
	 *
	 * @param cancelComponent
	 *            the control whose selection will trigger a cancel. This
	 *            parameter will be ignored and hence can be <code>null</code>
	 *            if a stop button was requested upon construction and instead
	 *            the stop button will enabled and serve as the cancel
	 *            component.
	 ...
	 * considered as a mnemonic character in SWT ToolItems, MenuItems, Button
	 * and Labels.
	 *
	 * @param in
	 *            the original String
	...
	protected void initialize(Layout layout, int progressIndicatorHeight) {
		...
		if (fHasStopButton) {
			...
			// listener operates on the fCancelComponent which must be a
			// control.
	 ...
	 *
	 * @param cancelComponent
	 *            the control that was previously used as a cancel component.
	 *            This parameter will be ignored and hence can be
	 *            <code>null</code> if a stop button was requested upon
	 *            construction and instead the stop button will be disabled.
	 * @see #ProgressMonitorPart(Composite, Layout, boolean)
	 */
	public void removeFromCancelComponent(Control cancelComponent) {
		if (fHasStopButton) {
			...
			Assert.isTrue(fCancelComponent == cancelComponent

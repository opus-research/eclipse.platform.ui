 ...
 *     Jeanderson Candido <http://jeandersonbc.github.io> - Bug 418192 - Added generics to this viewer.
 *******************************************************************************/
...
/**
 * A concrete tree-structured viewer based on an SWT <code>Tree</code> control
 * with checkboxes on each node.
 * <p>
 * This class supports setting an {@link ICheckStateProvider} to set the
 * checkbox states. To see standard SWT behavior, view SWT Snippet274.
 * </p>
 * <p>
...
public class CheckboxTreeViewer<E, I> extends TreeViewer<E, I> implements
	 ...
	 * <code>CHECK</code> and <code>BORDER</code>. The viewer has no input, no
	...
	/**
	 * Creates a tree viewer on a newly-created tree control under the given
	 * parent. The tree control is created using the given SWT style bits, plus
	 * the <code>CHECK</code> style bit. The viewer has no input, no content
	...
	/**
	 * Creates a tree viewer on the given tree control. The
	 * <code>SWT.CHECK</code> style bit must be set on the given tree control.
	 ...
	 *
	 * @param checked
	 *            a set of elements (element type: <code>Object</code>)
	 * @param grayed
	 *            a set of elements (element type: <code>Object</code>)
	 * @param widget
	 *            the widget
	 ...
	 *
	 * @param checked
	 *            a writable set of elements (element type: <code>Object</code>)
	 * @param grayed
	 *            a writable set of elements (element type: <code>Object</code>)
	 * @param widget
	 *            the widget
	...
	/**
	 * Sets the grayed state of all items to correspond to the given set of
	 * grayed elements.
	 *
	...
	/**
	 * Sets which elements are grayed in this viewer's tree. The given list
	 * contains the elements that are to be grayed; all other elements are to be
	 * ungrayed.
	 * <p>
	 * This method is typically used when restoring the interesting state of a
	 ...
	 * viewer. Does not fire events to check state listeners. Assumes that the
	 * element has been expanded before. To enforce that the item is expanded,
	 * call <code>expandToLevel</code> for the element.
	 *
	 * @param state
	 *            <code>true</code> if the element should be checked, and
	 *            <code>false</code> if it should be unchecked
	 * @deprecated as this method only checks or unchecks visible items is is
	 *             recommended that {@link #setSubtreeChecked(Object, boolean)}
	 *             is used instead.

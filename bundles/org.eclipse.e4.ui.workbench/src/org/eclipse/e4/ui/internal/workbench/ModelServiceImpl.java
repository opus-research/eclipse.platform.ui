...
public class ModelServiceImpl implements EModelService {
	...
	private <T> void findElementsRecursive(MApplicationElement searchRoot, Class<T> clazz,
		...
		if (searchFlags == ANYWHERE) {
			...
			if ((searchFlags & IN_ANY_PERSPECTIVE) != 0) {
				...
				el.addAll(findElementsByClass(searchRoot, MArea.class));
			} else if ((searchFlags & IN_PART) != 0) {
				el.addAll(findElementsByClass(searchRoot, MPart.class));
			}

			if ((searchFlags & IN_TRIM) != 0) {
				el.addAll(findElementsByClass(searchRoot, MTrimBar.class));
			}

			if ((searchFlags & IN_MAIN_MENU) != 0) {
				for (MApplicationElement e : findElementsByClass(searchRoot, MWindow.class)) {
					el.add(((MWindow) e).getMainMenu());
				}
			}

			// findElementsRecursive only on elements filtered by searchFlags
			for (MApplicationElement element : el) {
				findAllChildren(element, clazz, matcher, elements, searchFlags, true);
			}
		}
	}

	private <T> List<T> findElementsByClass(MApplicationElement searchRoot,
			Class<T> clazz) {

		List<T> elements = new ArrayList<T>();
		ElementMatcher matcher = new ElementMatcher(null, clazz, (String)null);
		findAllChildren(searchRoot, clazz, matcher, elements, ANYWHERE, false);
	...
	private <T> void findAllChildren(MApplicationElement searchRoot, Class<T> clazz,
		...
		TreeIterator<Object> itt = EcoreUtil.getAllContents((EObject) searchRoot, false);
		while (itt.hasNext()) {
			MApplicationElement element = (MApplicationElement) itt.next();
			...
			if (placeholderLooup) {
				if (element instanceof MPlaceholder) {
					...
					if (ph.getRef() != null
						...
						findAllChildren(ph.getRef(), clazz, matcher, elements, searchFlags, true);
					}
				}
			}
		}

...
public class ModelAssembler {
	 ...
	 *            <code>true</code> if running from a non-persisted state
	 */
	void processFragments(IExtension[] extensions, List<MApplicationElement> imports,
		...
		for (IExtension extension : extensions) {
			...
			for (IConfigurationElement ce : ces) {
				if ("fragment".equals(ce.getName()) && (initial || !INITIAL.equals(ce.getAttribute("apply")))) { //$NON-NLS-1$ //$NON-NLS-2$
					...
					processFragments(ce, checkExist, imports, addedElements);
				}
			}
		}
	}
	 ...
	 */
	public List<MApplicationElement> processFragment(MModelFragment fragment, String contributorURI,
			boolean checkExist) {
	 ...
	 *            processors
	 * @param initial
	 *            <code>true</code> if the application is runnign from a
	 *            non-persisted state
	...
	void runProcessor(IConfigurationElement ce) {

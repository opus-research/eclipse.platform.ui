...
public class CocoaUIHandler {
	...
	void log(Exception e) {
		...
		statusReporter.get().report(
				new Status(IStatus.WARNING, CocoaUIProcessor.FRAGMENT_ID,
	...
	@Inject
	@Optional
	private void subscribeApplicationTagChanged(@UIEventTopic(UIEvents.ApplicationElement.TOPIC_TAGS) Event event) {
		if (event.getProperty(UIEvents.EventTags.ELEMENT) instanceof MWindow) {
			MWindow window = (MWindow) event.getProperty(UIEvents.EventTags.ELEMENT);
			updateFullScreenStatus(window);
		}
	...
	private void redirectHandledMenuItem(MMenuItem item) {
		...
		if (elmtId != null
				&& (elmtId.equals(COMMAND_ID_ABOUT) || elmtId.equals(COMMAND_ID_PREFERENCES) || elmtId

...
public interface ISideEffectFactory {
	 ...
	 * </p>
	 * Callers who invoke this {@link #createFactory(Consumer)} method for the
	 * creation of an {@link ISideEffectFactory}, are supposed to manage the
	 * lifecycle of the aggregated {@link ISideEffect} instances, which are
	 ...
	 * not run it immediately. The lifecycle of the created {@link ISideEffect}
	 * is supposed to be managed by the caller of
	 * {@link #createFactory(Consumer)}. So for example pausing, resuming and
	 ...
	 * activate it immediately. The lifecycle of the created {@link ISideEffect}
	 * is supposed to be managed by the caller of
	 * {@link #createFactory(Consumer)}. So for example pausing, resuming and
	 * disposing should be done by the aggregating {@link Consumer} rather than
	 * on each individual {@link ISideEffect} itself.
	 ...
	 * lifecycle of the created {@link ISideEffect} is supposed to be managed by
	 * the caller of {@link #createFactory(Consumer)}. So for example pausing,
	 * resuming and disposing should be done by the aggregating {@link Consumer}
	 * rather than on each individual {@link ISideEffect} itself.
	 *
	 * @param runnable
	 ...
	 * realm. The lifecycle of the created {@link ISideEffect} is supposed to be
	 * managed by the caller of {@link #createFactory(Consumer)}. So for example
	 * pausing, resuming and disposing should be done by the aggregating

 ...
 * enters the disposed state it remains in that state until it is garbace
 * collected.</li>
 * </ul>
 *
 * Example usage:
...
public interface ISideEffect {
	 ...
	 * slightly more efficient than {@link #createAsync(Supplier, Consumer)} and
	 ...
	 *         executed anymore after the dispose method is invoked.
	 */
	static <T> ISideEffect createAsync(Supplier<T> supplier, Consumer<T> consumer) {
	 ...
	 *         paused.
	 */
	static <T> ISideEffect runOnceAsync(Supplier<T> supplier, Consumer<T> consumer) {

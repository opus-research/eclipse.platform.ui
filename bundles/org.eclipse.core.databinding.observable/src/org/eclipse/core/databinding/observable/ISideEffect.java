 ...
 * receive inputs from observables and produce output which are used by other
 ...
 * immediately on construction, useful for creating a side-effect in an object's
 * constructor which should not begin running until a later time. When using a
 * side-effect to update a control or a view, it is common to pause the
 * side-effect when the view is hidden and resume the side-effect when the view
 * becomes visible.</li>
 ...
 * </li>
 * <li>Disposed: The side-effect will not listen to or react to changes. Once a
 * side-effect enters the disposed state it cannot leave.</li>
 * </ul>
 *
 * Example usage:
 *
 ...
 * name. It will react automatically to changes in the username or the
 ...
 * void createControls() {
 *   ISideEffect sideEffect = ISideEffect.create(() -> {
 *       ObservableValue<Image> myAvatar = new AvatarObservable();
 *
 ...
 * // the side-effect.
 * void createControls() {
 *   final ObservableValue<Image> myAvatar = new AvatarObservable();
 *   ISideEffect sideEffect = ISideEffect.create(() -> {
 *       myIcon.setImage(myAvatar.getValue());
 *   });
 * }

...
public interface ISaveablePart2 extends ISaveablePart {
	...
	/**
	 * Standard return code constant (value 0) indicating that the part needs to
	 * be saved.
	 */
	...
	/**
	 * Prompts the user for input on what to do with unsaved data. This method
	 * is only called when the part is closed or when the Workbench is shutting
	 * down.
	 * <p>
	 * Implementors are expected to open a custom dialog where the user will be
	 * able to determine what to do with the unsaved data. Implementors may also
	 * return a result of <code>DEFAULT</code> to get the default prompt
	 * handling from the Workbench.
	 * </p>
	 * 

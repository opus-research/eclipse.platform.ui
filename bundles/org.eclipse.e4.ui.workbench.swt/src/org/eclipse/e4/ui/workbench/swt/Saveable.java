...
public abstract class Saveable extends InternalSaveable implements IAdaptable {
	 ...
	 * for any other reason, the part should invoke <code>setCancelled</code> on
	 * the <code>IProgressMonitor</code> to inform the caller. If the background
	 * part of the save is cancelled, the job should return a
	 ...
	 * currently being saved in response to closing a workbench part, in which
	 * case further changes to this saveable through the UI must be prevented.
	 * </p>
	 * <p>

...
import org.eclipse.e4.ui.internal.workbench.swt.dialog.about.ui.AboutBundleData;
import org.eclipse.e4.ui.internal.workbench.swt.dialog.about.ui.AboutUtils;
...
public class BundleSigningInfo {
	...
	public Control createContents(Composite parent) {
		...
		// date
		{
			...
			label.setText(WorkbenchSWTMessages.BundleSigningTray_Signing_Date);
			...
			gc.dispose();
			date.setText(WorkbenchSWTMessages.BundleSigningTray_Working);
			date.setLayoutData(data);
		}
		// signer
		{
			Label label = new Label(composite, SWT.NONE);
			label.setText(WorkbenchSWTMessages.BundleSigningTray_Signing_Certificate);
			...
			certificate.setText(WorkbenchSWTMessages.BundleSigningTray_Working);
	...
	private void startJobs() {
		...
		if (factoryRef == null) {
			AboutUtils.handleStatus(WorkbenchSWTMessages.BundleSigningTray_Cant_Find_Service, IStatus.WARNING);
			return;
		}
...
NLS.bind(WorkbenchSWTMessages.BundleSigningTray_Determine_Signer_For,
				myData.getId())) {
			...
			protected IStatus run(IProgressMonitor monitor) {
				try {
					...
					if (signers.length == 0) {
						signerText = WorkbenchSWTMessages.BundleSigningTray_Unsigned;
						dateText = WorkbenchSWTMessages.BundleSigningTray_Unsigned;
					} else {
						Properties[] certs = parseCerts(signers[0].getCertificateChain());
						if (certs.length == 0)
							signerText = WorkbenchSWTMessages.BundleSigningTray_Unknown;
						else {
						...
						if (signDate != null)
							...
							dateText = WorkbenchSWTMessages.BundleSigningTray_Unknown;
					}
		...
		Job cleanup = new Job(WorkbenchSWTMessages.BundleSigningTray_Unget_Signing_Service) {

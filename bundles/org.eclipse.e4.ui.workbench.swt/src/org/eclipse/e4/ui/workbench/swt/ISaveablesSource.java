...
public interface ISaveablesSource {
	 ...
	 * value of this method changes during the lifetime of this part (i.e. after
	 * initialization and control creation but before disposal) the part must
	 * notify an implicit listener using
	 ...
	 * <code>PRE_CLOSE</code> event may trigger prompts to save dirty saveables,
	 * the cancellation status of the event must be checked by the part after
	 * the notification. When removing only non-dirty saveables,
	 ...
	 * method. Parts that implement {@link IReusableEditor} must notify when
	 * their input is changed through

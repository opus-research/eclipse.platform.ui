...
import org.eclipse.e4.ui.internal.workbench.swt.dialog.about.ConfigurationInfo;
...
public final class AboutSystemPage extends ProductInfoPage {
	...
	public void createPageButtons(Composite parent) {
		Button button = createButton(parent, BROWSE_ERROR_LOG_BUTTON,
				WorkbenchSWTMessages.AboutSystemDialog_browseErrorLogName);
		String filename = Platform.getLogFileLocation().toOSString();
		button.setEnabled(new File(filename).exists());

		createButton(parent, COPY_TO_CLIPBOARD_BUTTON, WorkbenchSWTMessages.AboutSystemDialog_copyToClipboardName);
	}
	...
	private void fetchConfigurationInfo(final Text text) {
		text.setText(WorkbenchSWTMessages.AboutSystemPage_RetrievingSystemInfo);
		Job job = new Job(WorkbenchSWTMessages.AboutSystemPage_FetchJobTitle) {
			@Override

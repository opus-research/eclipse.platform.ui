...
import org.eclipse.e4.ui.workbench.swt.dialog.BundleSigningInfo;
import org.eclipse.e4.ui.workbench.swt.internal.copy.BasicUIJob;
...
public class AboutPluginsPage extends ProductInfoPage {
	...
	private String columnTitles[] = { WorkbenchSWTMessages.AboutPluginsDialog_signed,
			WorkbenchSWTMessages.AboutPluginsDialog_provider, WorkbenchSWTMessages.AboutPluginsDialog_pluginName,
			WorkbenchSWTMessages.AboutPluginsDialog_version, WorkbenchSWTMessages.AboutPluginsDialog_pluginId,
	...
	protected void handleSigningInfoPressed() {
		if (signingArea == null) {
			...
			signingInfo.setText(WorkbenchSWTMessages.AboutPluginsDialog_signingInfo_hide);

		} else {
			// hide
			signingInfo.setText(WorkbenchSWTMessages.AboutPluginsDialog_signingInfo_show);
			signingArea.dispose();
			signingArea = null;
	...
	public void createPageButtons(Composite parent) {
		...
		moreInfo = createButton(parent, MORE_ID, WorkbenchSWTMessages.AboutPluginsDialog_moreInfo);
		moreInfo.setEnabled(false);

		signingInfo = createButton(parent, SIGNING_ID, WorkbenchSWTMessages.AboutPluginsDialog_signingInfo_show);
		signingInfo.setEnabled(false);

		createButton(parent, COLUMNS_ID, WorkbenchSWTMessages.AboutPluginsDialog_columns);
	}
	...
	protected void createTable(Composite parent) {
		...
		filterText.setMessage(WorkbenchSWTMessages.AboutPluginsDialog_filterTextMessage);
		filterText.setFocus();
	...
	protected void handleMoreInfoPressed() {
		...
		if (!AboutUtils.openBrowser(getShell(), getMoreInfoURL(bundleInfo, true))) {
			String message = NLS.bind(WorkbenchSWTMessages.AboutPluginsDialog_unableToOpenFile, PLUGININFO,
					bundleInfo.getId());

			AboutUtils.handleStatus(WorkbenchSWTMessages.AboutPluginsDialog_errorTitle + ": " + message);
		}
	}

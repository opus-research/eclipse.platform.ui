...
package org.eclipse.e4.ui.internal.workbench.swt.dialog.about;
...
import org.eclipse.e4.ui.workbench.swt.dialog.ISystemSummarySection;
...
public final class ConfigurationInfo {
	...
	public static String getSystemSummary() {
		...
		writer.println(NLS.bind(WorkbenchSWTMessages.SystemSummary_timeStamp,
	...
	private static void appendExtensions(PrintWriter writer) {
		...
		for (int i = 0; i < configElements.length; ++i) {
			...
			try {
				...
				AboutUtils.handleStatus(WorkbenchSWTMessages.AboutDialog_CanNotCreateExtension, e);
			}

			writer.println();
			writer.println(NLS.bind(WorkbenchSWTMessages.SystemSummary_sectionTitle,
					element.getAttribute("sectionTitle"))); //$NON-NLS-1$

			if (obj instanceof ISystemSummarySection) {
				ISystemSummarySection logSection = (ISystemSummarySection) obj;
				logSection.write(writer);
			} else {
				writer.println(WorkbenchSWTMessages.SystemSummary_sectionError);
			}
		}
	}

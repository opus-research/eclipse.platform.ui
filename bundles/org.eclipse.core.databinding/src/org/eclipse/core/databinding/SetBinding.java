	...
	private void doUpdate(final IObservableSet source,
		...
		destination.getRealm().exec(() -> {
			...
			try {
				...
				for (Iterator iterator1 = diff.getRemovals().iterator(); iterator1
						.hasNext();) {
					IStatus setterStatus1 = updateSetStrategy.doRemove(
							destination,
				...
				for (Iterator iterator2 = diff.getAdditions().iterator(); iterator2
						.hasNext();) {
					IStatus setterStatus2 = updateSetStrategy.doAdd(
							destination,

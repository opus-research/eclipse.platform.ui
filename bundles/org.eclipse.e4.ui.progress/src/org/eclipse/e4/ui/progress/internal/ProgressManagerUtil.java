...
public class ProgressManagerUtil {
	...
	static void logException(Throwable exception) {
		ExternalServices.getLogService().log(LogService.LOG_ERROR,
	...
	public static void openProgressView() {
		MPart progressView = (MPart) ExternalServices.getModelService().find(
		        ProgressManager.PROGRESS_VIEW_NAME, ExternalServices.getMWindow());
		if (progressView == null) {
			return;
		}
		ExternalServices.getPartService().activate(progressView);
	}
	...
	public static boolean rescheduleIfModalShellOpen(Job openJob) {
		...
		openJob.schedule(ProgressManager.getInstance()
	...
	public static Shell getModalShellExcluding(Shell shell) {
		...
		if (shell == null || shell.isDisposed()) {
			return getModalChildExcluding(ExternalServices.getShell()
			        .getShells(), shell);
		}
	...
	public static Shell getNonModalShell() {
		MApplication application = ExternalServices.getMApplication();
		if (application == null) {
...
//
	...
	public static URL getIconsRoot() {
		//TODO E4
		// $nl$/icons/full/progress/
		try {
			return new URL("platform:/plugin/" + IProgressConstants.PLUGIN_ID
			        + "/icons/full/progress");

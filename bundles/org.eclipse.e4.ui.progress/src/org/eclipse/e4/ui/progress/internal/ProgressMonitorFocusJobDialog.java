...
import org.eclipse.e4.ui.progress.e4new.Preferences;
...
class ProgressMonitorFocusJobDialog extends ProgressMonitorJobsDialog {
	...
	public void show(Job jobToWatch, final Shell originatingShell) {
		...
		BusyIndicator.showWhile(ExternalServices.getDisplay(),
				new Runnable() {
					public void run() {
						try {
							Thread
	...
	protected void createExtendedDialogArea(Composite parent) {
		showDialog = Boolean.parseBoolean(Preferences
		        .get(IProgressConstants.RUN_IN_BACKGROUND));
	...
	public boolean close() {
		if (getReturnCode() != CANCEL) {
			Preferences.set(IProgressConstants.RUN_IN_BACKGROUND,
			        Boolean.toString(showDialog));
		}

...
public class ProgressInfoItem extends Composite {
	...
	static {
		JFaceResources
				...
				.put(
						STOP_IMAGE_KEY,
						getImageDescriptor("elcl16/progress_stop.gif"));//$NON-NLS-1$

		JFaceResources
				.getImageRegistry()
				.put(
						DISABLED_STOP_IMAGE_KEY,
		        		getImageDescriptor("dlcl16/progress_stop.gif"));//$NON-NLS-1$

		JFaceResources
				.getImageRegistry()
				.put(
						DEFAULT_JOB_KEY,
		        		getImageDescriptor("progress/progress_task.gif")); //$NON-NLS-1$

		JFaceResources
				.getImageRegistry()
				.put(
						CLEAR_FINISHED_JOB_KEY,
						getImageDescriptor("elcl16/progress_rem.gif")); //$NON-NLS-1$

		JFaceResources
				.getImageRegistry()
				.put(
						DISABLED_CLEAR_FINISHED_JOB_KEY,
		        		getImageDescriptor("dlcl16/progress_rem.gif")); //$NON-NLS-1$

		// Mac has different Gamma value
		int shift = Util.isMac() ? -25 : -10;

		Color lightColor = ExternalServices.getDisplay().getSystemColor(
	...
	public void executeTrigger() {
		...
		if (data instanceof IAction) {
			...
			EHandlerService handlerService = ExternalServices.getEHandlerService();
			handlerService.executeHandler((ParameterizedCommand) data);
		}

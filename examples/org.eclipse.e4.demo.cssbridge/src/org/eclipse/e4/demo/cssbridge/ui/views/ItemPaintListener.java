...
public abstract class ItemPaintListener<T extends Item> implements Listener {
	...
	public void handleEvent(Event event) {
		if (isSelected(event.detail)) {
			...
			event.gc.setForeground(Theme.getColor(Theme.Shell.SELECTION_FOREGROUND));
			event.gc.setBackground(Theme.getColor(Theme.Shell.SELECTION_BACKGROUND));									
			event.gc.fillRectangle(0, rec.y, getParentBounds(item).width, rec.height);
			...
			event.gc.drawText(text, event.x + calculateTextLeftPadding(item, event.index), 

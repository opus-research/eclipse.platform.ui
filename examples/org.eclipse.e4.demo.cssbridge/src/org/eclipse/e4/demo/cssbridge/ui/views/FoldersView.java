...
public class FoldersView extends ViewPart {
	...
	private Listener treeItemPaintListener = new ItemPaintListener<org.eclipse.swt.widgets.TreeItem>() {
		@Override
		protected String getText(org.eclipse.swt.widgets.TreeItem item, int index) {
			return item.getText(index);
		}

		@Override
		protected Rectangle getBounds(org.eclipse.swt.widgets.TreeItem item, int index) {
			return item.getBounds(index);
		}

		@Override
		protected Rectangle getParentBounds(org.eclipse.swt.widgets.TreeItem item) {
			return item.getParent().getBounds();
		}

		@Override
		protected Image getImage(org.eclipse.swt.widgets.TreeItem item, int index) {
			return item.getImage();
		}
		
		@Override
		protected int calculateTextLeftPadding(org.eclipse.swt.widgets.TreeItem item, int index) {
	...
	public void dispose() {
		if (!viewer.getTree().isDisposed()) {
			viewer.getTree().removeListener(SWT.PaintItem, treeItemPaintListener);
		}
	...
	private class ViewLabelProvider extends LabelProvider implements IFontProvider, IColorProvider {
		@Override
		...
		public Image getImage(Object elem) {
			...
			return sharedImages.getImage(isFolderType(elem)?
					ISharedImages.IMG_OBJ_ELEMENT: ISharedImages.IMG_OBJ_FOLDER);
		}

		@Override
		public Font getFont(Object element) {
			return isFolderType(element)?
					Theme.getFont(Theme.FoldersView.FOLDER_TYPE_FONT):
						Theme.getFont(Theme.FoldersView.MAILBOX_NAME_FONT);
		}
		...
		public Color getForeground(Object element) {
			return isFolderType(element)?
					Theme.getColor(Theme.FoldersView.FOLDER_TYPE_FOREGROUND):
						Theme.getColor(Theme.FoldersView.MAILBOX_NAME_FOREGROUND);
		}

...
public class SnippetSideEffectMigration {
	...
	static class TrackedView {
		...
		private void bindData() {
			...
			ISideEffect firstNameModelToTarget = ISideEffect.create(person::getFirstName, personFirstNameText::setText);

			WritableValue<IStatus> firstNameValidation = new WritableValue<>();
			ISideEffect firstNameTargetToModel = ISideEffect.create(() -> {
			...
			ISideEffect lastNameModelToTarget = ISideEffect.create(person::getLastName, personLastNameText::setText);

			WritableValue<IStatus> lastNameValidation = new WritableValue<>();
			ISideEffect lastNameTargetToModel = ISideEffect.create(() -> {
			...
			// dispose the ISideEffect objects, when the widget is disposed
			personFirstNameText.addDisposeListener(e -> {
				firstNameModelToTarget.dispose();
				firstNameTargetToModel.dispose();
				lastNameModelToTarget.dispose();
				lastNameTargetToModel.dispose();
			});
		}
	}

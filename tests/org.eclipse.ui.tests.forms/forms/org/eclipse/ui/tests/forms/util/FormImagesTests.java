...
public class FormImagesTests extends TestCase {
	public void testSingleton() {
		...
		assertTrue("getInstance() returned a different FormImages instance after creation and disposal of one image", instance.equals(FormImages.getInstance()));
	}
	...
	public void testMultipleSimpleInstances() {
		...
		for ( ;count > 0; count--) {
			...
			if (count != 1)
				...
				assertFalse("markFinished(...) disposed a shared image early on iteration "+count,gradient.isDisposed());
			else
	...
	public void testMultipleUniqueInstances() {
		...
		for (int i = 0; i < images.length - 1; i++) {
			for (int j = i+1; j < images.length; j++) {
				assertNotSame("getGradient(...) returned the same image for different parameters: i = " + i + "; j = " + j, images[i], images[j]);
			}
		}
		...
		for (int i = 0; i < images.length; i++) {
			...
			assertTrue("markFinished(...) did not dispose an image that was only requested once: i = " + i, images[i].isDisposed());
		}
	}
	...
	public void testComplexEquality() {
		...
		assertEquals("different images were created with only the background color differing when that difference is irrelevant", image1, image2);

...
public class SizeCacheTest extends TestCase {
	...
	private Point checkSizeEquals(int whint, int hhint) {
		...
		sizeCache.setControl(null);
		sizeCache.setControl(control);
		update();
		Point calcSize = control.computeSize(whint, hhint);


		if (whint != SWT.DEFAULT && hhint != SWT.DEFAULT) {
			int bw = control.getBorderWidth();
			Point expected = new Point(whint + bw * 2, hhint + bw * 2);
			boolean goodControl = expected.equals(calcSize);
			// if control ignores hints we cannot cache it currently properly
			if (!goodControl) {
				System.err.println("Control mostly ignores hints " + control + " " + expected + " vs " + calcSize);
				calcSize = expected;
			}
		}
		...
		cachedSize = sizeCache.computeSize(whint, hhint);
		assertEquals(calcSize, cachedSize);
		// this is just for show if somebody is watching
		control.setSize(calcSize);
		dispatch();

		// now we use cache to get preffered size first, that uses different
		// code branches
		sizeCache.setControl(null);
		sizeCache.setControl(control);

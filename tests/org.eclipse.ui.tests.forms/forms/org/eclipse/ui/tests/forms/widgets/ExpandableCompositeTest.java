...
public class ExpandableCompositeTest extends TestCase {
	...
	// change this to true if you want to see graphics
	...
	public static Point getTextExtend(String str) {
	...
	private static void dispatch(int msec) {
		...
		do {
			...
			if (pass < msec) {
				// not doing display.sleep() because its automated tests, nobody
				// will cause any display evetns
				try {
					Thread.sleep(delay);
	...
	public void testExpCompNoClient() {
		...
		// createClient();
		// Point tsize = getTextExtend(TEXT1);
	...
	public void testExpCompWithClient() {
		...
		ec.setExpanded(true);
		Rectangle bounds2 = update();
		// our client is 100 tall + ver specing

...
public class WorkbenchThemeChangedHandlerTest extends TestCase {
	public void testOverrideThemeDefinitions() throws Exception {
		...
		ThemeRegistryTestable themeRegistry = spy(new ThemeRegistryTestable());
		...
		//then
		verify(fontRegistry, never()).remove(any(String.class));
		verify(colorRegistry, never()).remove(any(String.class));
		verify(themeRegistry, never()).remove(any(ColorDefinition.class));
		verify(themeRegistry, never()).remove(any(FontDefinition.class));
	...
	public void testResetThemeRegistries() throws Exception {
		...
		FontDefinition fontDefinition = mock(FontDefinition.class);
		doReturn("fontDefinition").when(fontDefinition).getId();
		doReturn(false).when(fontDefinition).isOverridden();
		doReturn(true).when(fontDefinition).isAddedByCss();
		
		ColorDefinition colorDefinition = mock(ColorDefinition.class);
		doReturn("colorDefinition").when(colorDefinition).getId();
		doReturn(false).when(colorDefinition).isOverridden();
		doReturn(true).when(colorDefinition).isAddedByCss();
		
		ThemeRegistryTestable themeRegistry = spy(new ThemeRegistryTestable());
		doReturn(new FontDefinition[]{fontDefinition}).when(themeRegistry).getFonts();
		doReturn(new ColorDefinition[] {colorDefinition}).when(themeRegistry).getColors();
		...
		//then
		verify(fontRegistry, times(1)).remove(any(String.class));
		verify(colorRegistry, times(1)).remove(any(String.class));
		verify(themeRegistry, times(1)).remove(any(ColorDefinition.class));
		verify(themeRegistry, times(1)).remove(any(FontDefinition.class));
	}
	...
	public static class ThemeRegistryTestable extends ThemeRegistry {
		@Override
		public void remove(ColorDefinition desc) {
			super.remove(desc);
		}
		@Override

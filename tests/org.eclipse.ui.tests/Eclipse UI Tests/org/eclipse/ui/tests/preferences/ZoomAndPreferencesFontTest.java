...
public class ZoomAndPreferencesFontTest {
	...
	public void openEditor() throws Exception {
		...
		this.initialFontHeight = text.getFont().getFontData()[0].getHeight();
	}
	...
	public void restoreAndCheckDefaults() {
		...
		Assert.assertEquals(this.initialFontHeight, text.getFont().getFontData()[0].getHeight());
		editor.close(false);
	}
	...
	public void testZoomCommand() throws Exception {
		...
		Assert.assertEquals(this.initialFontHeight + times * 2, text.getFont().getFontData()[0].getHeight());
	}

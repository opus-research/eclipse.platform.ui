...
public class AutoSavePreferencePageTest {
	...
	/**
	 * The default interval auto-save value.
	 */
	...
	public void restoreAndCheckDefaults() {
		...
		Assert.assertEquals(DEFAULT_VALUE, autoSaveInterval);
		Assert.assertEquals(DEFAULT_VALUE, page.getAutoSaveIntervalTextValue());
	}
	...
	public void testUpdateAutoSaveInterval() {
		...
		// performOk, so the value must be DEFAULT_VALUE before the perform and
		// 30 after
		// the perform.
		Assert.assertEquals(DEFAULT_VALUE, autoSaveInterval);
	...
	public void testUpdateAutoSaveIntervalWithOutOfScopeValue() {
		...
		int autoSaveInterval = WorkbenchPlugin.getDefault().getPreferenceStore()
				.getInt(IPreferenceConstants.SAVE_AUTOMATICALLY_INTERVAL);
		// the value must be DEFAULT_VALUE before the perform and DEFAULT_VALUE
		// after

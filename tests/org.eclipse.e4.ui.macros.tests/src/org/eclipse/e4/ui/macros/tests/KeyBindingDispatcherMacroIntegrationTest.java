...
import org.eclipse.e4.core.macros.internal.MacroContextImplementation;
...
public class KeyBindingDispatcherMacroIntegrationTest {
	...
	public void setUp() {
		...
		workbenchContext.set(EMacroContext.class.getName(), new MacroContextServiceCreationFunction());
		...
		((MacroContextImplementation) macroContext).getMacroManager().setMacrosDirectories(fMacrosDirectory);
	}
	...
	public void testMacroIntegrationSaveRestore() throws Exception {
		...
		((MacroContextImplementation) macroContext).getMacroManager().reloadMacros();
		handler.q2 = false;
		macroContext.playbackLastMacro(new IMacroPlaybackContext() {
		});
		assertTrue(handler.q2);

...
public class EModelServiceFindTest extends TestCase {
	...
	public void testFindElementsIdOnly() {
		...
		elements = modelService.findElements(application, null, EModelService.ANYWHERE | EModelService.IN_MAIN_MENU | EModelService.IN_PART, getSelector("toolControl1Id"));
		assertEquals(1, elements.size());
	}
	...
	public void testFindElementsTypeOnly() {
		...
		elements = modelService.findElements(application, MPart.class, EModelService.ANYWHERE, getSelector(MPart.class));
		assertEquals(5, elements.size());

		elements = modelService.findElements(application, MPartStack.class, EModelService.ANYWHERE, getSelector(MPartStack.class));
		assertEquals(3, elements.size());

		List<MDirtyable> dirtyableElements = modelService.findElements(application, MDirtyable.class, EModelService.ANYWHERE, getSelector(MDirtyable.class));
		assertEquals(5, dirtyableElements.size());

		elements = modelService.findElements(application, MMenuElement.class, EModelService.ANYWHERE | EModelService.IN_MAIN_MENU | EModelService.IN_PART, getSelector(MMenuElement.class));
		assertEquals(13, elements.size());

		elements = modelService.findElements(application, MToolBarElement.class, EModelService.ANYWHERE | EModelService.IN_MAIN_MENU | EModelService.IN_PART, getSelector(MToolBarElement.class));
		assertEquals(2, elements.size());

		// Should find all the elements
		elements = modelService.findElements(application,null, EModelService.ANYWHERE | EModelService.IN_MAIN_MENU | EModelService.IN_PART, getSelector());
	...
	public void testFindElementsTagsOnly() {
		...
		List<MUIElement> oneTags = modelService.findElements(application, null, EModelService.ANYWHERE, getSelector(tags));
		assertEquals(oneTags.size(), 1);

		tags.clear();
		tags.add("twoValidTags");
		List<MUIElement> twoTags = modelService.findElements(application, null, EModelService.ANYWHERE, getSelector(tags));
		assertEquals(twoTags.size(), 2);

		tags.clear();
		tags.add("invalidTag");
		List<MUIElement> invalidTags = modelService.findElements(application, null, EModelService.ANYWHERE, getSelector(tags));
		assertEquals(invalidTags.size(), 0);

		tags.clear();
		tags.add("twoValidTags");
		tags.add("secondTag");
		List<MUIElement> combinedTags = modelService.findElements(application, null, EModelService.ANYWHERE, getSelector(tags));
		assertEquals(combinedTags.size(), 1);

		tags.clear();
		tags.add("oneValidTag");
		tags.add("secondTag");
		List<MUIElement> unmatchedTags = modelService.findElements(application, null, EModelService.ANYWHERE, getSelector(tags));
	...
	public void testFindElementsCombinations() {
		...
		List<MPartSashContainer> typeAndTag = modelService.findElements(application, MPartSashContainer.class, EModelService.ANYWHERE, getSelector(tags));
		assertEquals(1, typeAndTag.size());

		List<MUIElement> idAndTag = modelService.findElements(application, null, EModelService.ANYWHERE, getSelector("twoValidIds", null, tags));
		assertEquals(1, idAndTag.size());

		List<MPartSashContainer> idAndTypeAndTags = modelService.findElements(application, MPartSashContainer.class, EModelService.ANYWHERE, getSelector("twoValidIds", MPartSashContainer.class, null));
		assertEquals(1, idAndTypeAndTags.size());

		List<MPartSashContainer> badIdAndTypeAndTags = modelService.findElements(application, MPartSashContainer.class, EModelService.ANYWHERE, getSelector("invalidId"));
	...
	public void testFlags() {
		...
		elements = modelService.findElements(application, clazz, EModelService.IN_ANY_PERSPECTIVE | EModelService.IN_PART, getSelector(clazz));
		assertEquals(0, elements.size());
		...
		elements = modelService.findElements(application, clazz, EModelService.IN_ACTIVE_PERSPECTIVE | EModelService.IN_PART, getSelector(clazz));
		assertEquals(3, elements.size());

		elements = modelService.findElements(application, clazz, EModelService.IN_ANY_PERSPECTIVE | EModelService.IN_MAIN_MENU, getSelector(clazz));
		assertEquals(13, elements.size());

		elements = modelService.findElements(application, clazz, EModelService.IN_MAIN_MENU, getSelector(clazz));
		assertEquals(9, elements.size());
	}

	public void testFind_Handler() {
		MApplication application = createApplication();
	...
	public void testFind_MBindingContext() {
		...
		elements = modelService.findElements(application, clazz, EModelService.ANYWHERE, getSelector( "org.eclipse.ui.contexts.window", clazz, null));
		assertEquals(1, elements.size());
